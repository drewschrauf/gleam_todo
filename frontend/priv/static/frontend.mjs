      ...
      return field(
        ...
        (description) => {
          return field(
            ...
            bool,
            (done) => {
              return success(new Task(id, description, done));
            }
          );
        }
      );
...
function task_list_decoder() {
  return list2(task_decoder());
}

// build/dev/javascript/frontend/remote_data.mjs
var NotRequested = class extends CustomType {
};
...
function map6(remote_data, map7) {
  if (remote_data instanceof Success) {
    let data = remote_data[0];
    return new Success(map7(data));
  } else {
    return remote_data;
  }
}
...
var Model = class extends CustomType {
  constructor(tasks, new_task_description, submitting) {
    super();
    this.tasks = tasks;
    this.new_task_description = new_task_description;
    this.submitting = submitting;
  }
};
var GotTasks = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var UserUpdatedNewTaskDescription = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var UserClickedSubmit = class extends CustomType {
};
var NewTaskSubmitted = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function init(_) {
  return [
    new Model(new Loading(), "", false),
    get2(
      "http://localhost:4444/tasks",
      expect_json(
        task_list_decoder(),
        (var0) => {
          return new GotTasks(var0);
        }
      )
    )
  ];
...
function update2(model, msg) {
  if (msg instanceof GotTasks) {
  ...
  } else if (msg instanceof UserUpdatedNewTaskDescription) {
    let new_task_description = msg[0];
    return [
      new Model(model.tasks, new_task_description, model.submitting),
      none()
    ];
  } else if (msg instanceof UserClickedSubmit) {
    let $ = model.new_task_description;
    if ($ === "") {
      return [model, none()];
    } else {
      let description = $;
      return [
        new Model(model.tasks, model.new_task_description, true),
        post(
          "http://localhost:4444/tasks",
          object2(
            toList([
          ...
          expect_json(
            task_decoder(),
            (var0) => {
              return new NewTaskSubmitted(var0);
            }
          )
        )
      ];
    ...
    if (response instanceof Ok) {
      ...
      return [
        new Model(
          (() => {
            let _pipe = model.tasks;
            return map6(
              _pipe,
              (_capture) => {
                return append(_capture, toList([task]));
              }
            );
          })(),
          "",
...
function view(model) {
  return div(
    toList([]),
    toList([
      h1(toList([]), toList([text2("Tasks!")])),
      (() => {
        let $ = model.tasks;
        if ($ instanceof NotRequested) {
          ...
          return div(
            ...
            toList([
                ...
                toList([
                  ...
                  input(
                    toList([
                      ...
                      value(model.new_task_description),
                      on_input(
                        (var0) => {
                          return new UserUpdatedNewTaskDescription(var0);
                        }
                      )
                    ])
                  )
              ...
              button(
                toList([
                  on_click(new UserClickedSubmit()),
                  disabled(model.submitting)
                ]),
                toList([text2("Submit")])
              )
          ...
          return text2("Something went wrong");
        }
      })()
    ])
  );
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 369, end: 418, pattern_start: 380, pattern_end: 385 }
    );
  }
  return void 0;
}
